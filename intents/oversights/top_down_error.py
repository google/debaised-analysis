...
https://docs.python.org/3/library/datetime.html#strftime-and-strptime-behavior
    ...
    for column in operational_dimensions:
        ...
        if column not in dimension_list:
            ...
            new_suggestion =  _check_top_down_error(initial_result_table,
                                                      new_result_table, 
                                                      column, slice1,
                                                      dimensions_len)
...
def _check_top_down_error(initial_result_table, new_result_table, new_added_column,
    ...
    while initial_row_i < initial_num_rows:
        if initial_row_i == initial_num_rows - 1 or initial_table_matrix[initial_row_i][:dimensions_len] != initial_table_matrix[initial_row_i + 1][:dimensions_len]:
            ...
            new_max_corelation = 0
            while new_row_i < new_num_rows and initial_table_matrix[initial_row_i][:dimensions_len] == new_table_matrix[new_row_i][:dimensions_len]:
                if new_row_i == new_row_i - 1 or new_table_matrix[new_row_i][:dimensions_len + 1] != new_table_matrix[new_row_i + 1][:dimensions_len + 1]:
                    max_disimilarity = constants.TOP_DOWN_ERROR_DISSIMILARITY_THRESHOLD
                else:
                    corelation = _calculate_relation(new_table_matrix[new_row_i][dimensions_len + 2], new_table_matrix[new_row_i + 1][dimensions_len + 2])
                    new_max_corelation = max(corelation, new_max_corelation)
                    new_row_i = new_row_i + 1
                new_row_i = new_row_i + 1

            initial_corelation = _calculate_relation(initial_table_matrix[initial_row_i][dimensions_len + 1], initial_table_matrix[initial_row_i + 1][dimensions_len + 1])

            if new_max_corelation >= constants.TOP_DOWN_ERROR_DISSIMILARITY_THRESHOLD and initial_corelation <= constants.TOP_DOWN_ERROR_SIMILARITY_THRESHOLD:
    ...
    if len(suggestion_row_list) == 0:
        ...
        new_suggestion = {}
        new_suggestion['suggestion'] = 'both the value is almost same in some grouping but different if we add ' + new_added_column + ' in initial grouping.'
