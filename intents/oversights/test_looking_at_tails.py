...
def test_1():
    ...
    print(suggestions)
    expected_suggestions = """{'suggestion': 'Values in these columns for the top-k rows are similar for other rows also', 'oversight_name': 'Looking at tails to find causes', 'is_column_level_suggestion': True, 'col_list': [{'column': 'games', 'confidence_score': 0.5549613585989281}]}"""
...
def test_2():
    ...
    print(suggestions)
    expected_suggestions = """{'suggestion': 'Values in these columns for the top-k rows are similar for other rows also', 'oversight_name': 'Looking at tails to find causes', 'is_column_level_suggestion': True, 'col_list': [{'column': 'Gender', 'confidence_score': 0.05474928826225144}]}"""
